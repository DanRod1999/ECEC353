...
{
    unsigned int i, j, idx, res;
    idx = 0;
    int num_elements = 0;

    /* Captilize buffer contents */
    ...
    /* Modified buffer contents  */
    for (i = 0; i < num_elements; i++) {
        res = ptrace(PTRACE_POKEDATA, pid, (void *) (address + i * sizeof (long) / 2), *(int *)(buffer +  i * sizeof (long) / 2));
            if (res == -1) perror("ptrace pokedata\n");
        for (j = 0; j < sizeof (long); j++)
            idx++;
    }
